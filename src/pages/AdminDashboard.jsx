...
export default function AdminDashboard() {
  ...
  useEffect(() => { if (activeTab === 'rates' && rates.length === 0) loadRates() }, [activeTab])
  ...
  const tabs = ['overview', 'transactions', 'users', 'maintenance', 'rates']
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50">
                    {['Date', 'User', 'Region', 'Benefits', 'Monthly', 'Actions'].map(h => (
                ...
                <tbody>
                  {stats.recentTransactions.map(t => (
                    <tr key={t.id} className="border-b border-gray-50 hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-gray-500 text-xs">{t.userEmail || '—'}</td>
